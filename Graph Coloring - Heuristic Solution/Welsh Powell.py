...
def DrawGraph(G,col_val):
	...
	values = []
	for node in G.nodes():
		values.append(col_val.get(node, color[col_val.get(node)]))
	nx.draw(G, with_labels = True, node_color = values, edge_color = 'black' ,width = 1, alpha = 0.7)  #with_labels=true is to show the node number in the output graph
